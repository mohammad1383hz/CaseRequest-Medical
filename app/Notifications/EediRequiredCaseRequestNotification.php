...
class EediRequiredCaseRequestNotification extends Notification
{
    use Queueable;
    ...
    {
        $this->statusNotification = StatusNotification::where('name','EediRequiredCaseRequest')->first();
        $this->user = $user;
   ...
   /**
    ...
    public function toSms(object $notifiable)
    {
        if ($this->statusNotification->sms) {
            $title=$this->statusNotification->title;
            $description=$this->statusNotification->description;
           SmsController::sendDefault($this->user->phone,124123);                
        }
