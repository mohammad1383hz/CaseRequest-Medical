...
class CaseReportNotification extends Notification
{
    ...
    public function __construct($user)
    {
        ...
        $this->statusNotification = StatusNotification::where('name','CaseReport')->first(); // اینجا یک مثال است، شما می‌توانید شناسه مورد نظر خود را استفاده کنید
    }
    ...
    public function toSms(object $notifiable)
    {
        if ($this->statusNotification->sms) {
            $title=$this->statusNotification->title;
            $description=$this->statusNotification->description;
           SmsController::sendDefault($this->user->phone,124100);                
        }
